...
> *Transforming functional 2D barcodes into deterministic 3D architectural worlds & voxel globes without sacrificing scannability.*
...
**JiwoQR** adalah ekosistem generasi baru untuk menghasilkan QR code 3D prosedural yang sepenuhnya interaktif dan deterministik. Dikembangkan dengan arsitektur monorepo modern, JiwoQR menggabungkan keindahan estetika *cyber-brutalist skyscraper* dan *dual-hemisphere voxel mound globe* dengan kepatuhan penuh terhadap standar internasional **ISO/IEC 18004**.
...
1. **100% Deterministic DNA**: Setiap payload/URL menghasilkan struktur kota atau bola voxel yang unik namun konsisten setiap kali dirender.
...
3. **Multi-Platform Ready**: Tersedia sebagai engine WebGL murni, komponen React siap pakai, serta Custom Element native tanpa dependensi framework.
...
- **Standar Penuh ISO/IEC 18004**:
  - Enkoder bitstream QR murni TypeScript (Mode Byte 8-bit).
...
JiwoQR menyediakan dua model visual utama yang dapat diganti secara dinamis:
...
├── packages/
│   ├── core/                      # Bitstream encoder, RS ECC, DNA generator
│   ├── math/                      # Vektor, easing, ekstrusi & proyeksi spherical
│   ├── renderer-webgl/            # Engine Three.js, instanced meshes & models
│   ├── react/                     # Komponen wrapper <JiwoQR /> untuk React
│   ├── web-component/             # Native Custom Element <jiwo-qr>
...
```bash
git clone https://github.com/username/jiwoQR.git
cd jiwoQR
pnpm install
```
...
<body>
  ...
  <script>
    ...
    // Ganti mode lewat method API DOM
...
- [`@jiwoqr/core` Documentation](file:///d:/REPOS/jiwoQR/packages/core/README.md) - Bitstream encoder, Galois Field, RS ECC, and Deterministic DNA.
- [`@jiwoqr/math` Documentation](file:///d:/REPOS/jiwoQR/packages/math/README.md) - Extrusions, spherical projections, easing curves, and interpolation math.
- [`@jiwoqr/renderer-webgl` Documentation](file:///d:/REPOS/jiwoQR/packages/renderer-webgl/README.md) - Three.js WebGL rendering engine, models, and camera controllers.
- [`@jiwoqr/react` Documentation](file:///d:/REPOS/jiwoQR/packages/react/README.md) - React component wrapper and hook synchronization.
